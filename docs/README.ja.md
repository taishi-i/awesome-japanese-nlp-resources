...
```shell
/awesome-japanese-nlp-resources:search 日本語NLPを学び始める初心者におすすめのチュートリアルは何ですか?
```

出力には **ユースケース選択ガイド** の表が含まれます:

| ユースケース | おすすめ | 人気度 | 理由 |
|---|---|---|---|
| 最新のNLP/LLMの概念をゼロから学ぶ | [llm-book](https://github.com/ghmagazine/llm-book) | ⭐387 | 最も人気のある入門書。実践的な日本語コード例とともにLLMを総合的に解説 |
| 演習を通じて実践的なスキルを習得する | [nlp100v2025](https://github.com/upura/nlp100v2025) | ⭐82 | 定番の「100本ノック」シリーズ。手を動かして学ぶのに最適 |
 ...
 * [lindera-sqlite](https://github.com/lindera/lindera-sqlite) - SQLite FTS5拡張機能用のLindera
 ...
 * [azooKey](https://github.com/azooKey/azooKey) - azooKeyは、iPhoneとiPad向けのオープンソースの日本語キーボードで、Swiftで書かれており、独自のかな漢字変換エンジンによって動作します。リアルタイム変換、柔軟なキーレイアウト、そしてスムーズなタイピング体験のためのクリーンなSwiftUIインターフェースを提供します。
 * [azooKey-Desktop](https://github.com/azooKey/azooKey-Desktop) - azooKey-Desktopは、macOS向けのオープンソースの日本語入力メソッドであり、Swiftで書かれており、Zenzaiニューラル仮名漢字変換エンジンによって動作します。ライブ変換、オプションのLLMベースの「マジック変換」、そしてTunerによるパーソナライゼーションを提供し、スムーズなデスクトップ入力体験を実現します。
 ...
 * [japanese-llm-benchmark](https://github.com/shi3z/japanese-llm-benchmark) - さまざまなLLMの日本語能力を評価するためのベンチマークツール。
 * [EDINET-Bench](https://github.com/SakanaAI/EDINET-Bench) - [ICLR 2026] 日本の難しい金融タスクにおけるLLMのパフォーマンス評価。
 * [LCTG-Bench](https://github.com/CyberAgentAILab/LCTG-Bench) - LCTGベンチ：LLM制御テキスト生成ベンチマークLCTGベンチ：LLM制御テキスト生成ベンチマーク
 * [Kokoro-Speech-Dataset](https://github.com/kaiidams/Kokoro-Speech-Dataset) - 公共ドメインの単一話者日本語音声データセット
 ...
 * [ttslearn](https://github.com/r9y9/ttslearn) - Input: ttslearn: Library for Pythonで学ぶ音声合成 (Pythonによるテキスト読み上げ)Output: ttslearn: Pythonのためのライブラリを使用して学ぶ音声合成 (Pythonによるテキスト読み上げ)
